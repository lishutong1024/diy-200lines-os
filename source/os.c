...
uint32_t task0_pg0_table[1024] __attribute__((aligned(4096)));
uint32_t task0_pg_dir[1024] __attribute__((aligned(4096))) = {
...
uint32_t task0_tss[] = {  
    ...
    (uint32_t)task0_pg_dir,  (uint32_t)task_0/*入口地址*/, 0x202, 0xa, 0xc, 0xd, 0xb, (uint32_t)task1_dpl0_stack + 4*1024/* 栈 */, 0x1, 0x2, 0x3,    
...
uint32_t task1_tss[] = {
    ...
    // cr3, eip, eflags, eax, ecx, edx, ebx, esp, ebp, esi, edi, 
    (uint32_t)task0_pg_dir,  (uint32_t)task_1/*入口地址*/, 0x202, 0xa, 0xc, 0xd, 0xb, (uint32_t)task1_dpl3_stack + 4*1024/* 栈 */, 0x1, 0x2, 0x3,    
    // es, cs, ss, ds, fs, gs, ldt, iomap
    APP_DATA_SEG, APP_CODE_SEG, APP_DATA_SEG, APP_DATA_SEG, APP_DATA_SEG, APP_DATA_SEG, 0x0, 0x0,
};
...
void os_start (void) {
    ...
    task0_pg_dir[MAP_ADDR >> 22] = (uint32_t)task0_pg0_table | PDE_P | PDE_W | PDE_U;
    task0_pg0_table[(MAP_ADDR >> 12) & 0x3FF] = (uint32_t)map_phy_buffer| PDE_P | PDE_W | PDE_U;
} 